...
RSpec.describe "/books/total", type: :request do
  ...
  describe "GET /books/total" do
    it "returns total books, total borrowed, and total due today" do
      get books_total_url, headers: auth_headers, as: :json
      expect(response).to be_successful
      expect(response.parsed_body["total_books"]).to eq(Book.sum(:copies))
      expect(response.parsed_body["total_borrowed"]).to eq(Borrowing.not_returned.count)
      expect(response.parsed_body["total_due_today"]).to eq(Borrowing.due_today.count)
    end
  end
end
